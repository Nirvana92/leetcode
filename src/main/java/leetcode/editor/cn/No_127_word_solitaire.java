 ...
 *
 * <p>
 * 这个版本的代码在leetcode 提交提示超时
 */
public class No_127_word_solitaire {
    @Test
    public void test() {
        // hit -> hot -> dot -> dog -> cog
...
//        beginWord = "teach";
//        endWord = "place";
//        wordList = Arrays.asList("peale", "wilts", "place", "fetch", "purer", "pooch", "peace", "poach", "berra", "teach", "rheum", "peach");
...
//        System.out.println(ladderLength);
    ...
    /**
     * 校验 wordList 中是否包含 endWord 中
     *
     * @param endWord
     * @param wordList
     * @return
     */
    boolean valid(String endWord, List<String> wordList) {
    ...
    /**
     * 宽度优先遍历: 还有一种优化的代码, 判断 start 和 end 层的那层的数量比较少, 从比较少的位置开始遍历
     *
     * @param beginWord
     * @param endWord
     * @param wordList
    ...
    public int ladderLengthBFS(String beginWord, String endWord, List<String> wordList) {
        if (!valid(endWord, wordList)) {
            return 0;
        }

        // 找到每个字符串的下一层字符串
        List<String> beginOneStepDis = loadOneStepDisLists(beginWord, wordList);
        Map<String, List<String>> oneStepDistanceMaps = new HashMap<>();
        oneStepDistanceMaps.put(beginWord, beginOneStepDis);
        for (int i = 0; i < wordList.size(); i++) {
            String startWord = wordList.get(i);
            if (!oneStepDistanceMaps.containsKey(startWord)) {
                oneStepDistanceMaps.put(startWord, loadOneStepDisLists(startWord, wordList));
            }
        }

        HashSet<String> startSets = new HashSet<>();
        // 1. 如果需要优化, 则通过使用两边往中间找的方式减少多余的查找判断,
        // 2. 如果字符串长度比较短, 可以枚举每个字符串的可能, 或者使用 * 表示每个可变的字符
        Set<String> endSets = new HashSet<>();
        Set<String> visited = new HashSet<>();
        startSets.add(beginWord);

        int level = 0;
        while (!startSets.isEmpty()) {
            HashSet<String> nextSets = new HashSet<>();

            for (String str : startSets) {
                if (!visited.contains(str)) {
                    ...
                    visited.add(str);
                    nextSets.addAll(oneStepDistanceMaps.get(str));
                }
            }
    ...
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        // beginWord 一步距离能到达的 wordList 中的字符串的映射
        // long initDatasStartTime = System.currentTimeMillis();
        List<String> beginOneStepDis = loadOneStepDisLists(beginWord, wordList);
        ...
        // long initDatasEndTime = System.currentTimeMillis();
        // System.out.println("===初始化数据耗时: " + (initDatasEndTime - initDatasStartTime));
        ...
        dp.put(endWord, 1);
        int minLen = process(beginWord, endWord, oneStepDistanceMaps, paths, dp);
        //System.out.println(dp);
        return minLen == Integer.MAX_VALUE ? 0 : minLen;
    }

    /**
     * 从 startWord 到 endWord 的最大距离
     *
     * @param startWord
     * @param oneStepDistanceMaps
     * @param paths
     * @return
     */
    int process(String startWord, String endWord, Map<String, List<String>> oneStepDistanceMaps, Set<String> paths, Map<String, Integer> dp) {
        ...
        if (dp.containsKey(startWord)) {
            return dp.get(startWord);
        }

        // startWord 到达的一步距离集合
        List<String> diss = oneStepDistanceMaps.get(startWord);
        // 记录最短距离
        int minLen = Integer.MAX_VALUE;
        for (String str : diss) {
            // str 没有遍历过
            // todo: bug, 因为这边会过滤某些字符, 导致有可能startWord 开始的字符串步数为 MAX_VALUE
            if (!paths.contains(str)) {
                // 如果到达了结束的字符串
                paths.add(str);
                int tmpMinLen = process(str, endWord, oneStepDistanceMaps, paths, dp);
                if (tmpMinLen != Integer.MAX_VALUE) {
                    minLen = Math.min(minLen, tmpMinLen + 1);
                }
        ...
        //System.out.println(startWord + ": " + minLen);
        if (minLen != Integer.MAX_VALUE) {
            dp.put(startWord, minLen);
        }
    ...
    /**
     * startWord 能一步到到的字符串保存到结果集合中返回
     *
     * @param startWord
     * @param wordList
     * @return
     */
    List<String> loadOneStepDisLists(String startWord, List<String> wordList) {
        List<String> oneStepDis = new ArrayList<>();
        for (String str : wordList) {
            if (oneStep(startWord, str)) {
                oneStepDis.add(str);
            }
        }
    ...
    boolean oneStep(String word, String targetWord) {
...
//        if (diffChar == 1) {
//            if (word.equals("acne") || targetWord.equals("acne")) {
//                System.out.println("word: " + word + "; targetWord: " + targetWord);
//            }
//        }
